...
permafrost_type = ['RS_2002_permafrost', 'RS_2002_all']
############# Loop through data ###########
...
###### Overall (across all validation grid cells) ######
    					...
    					dframe_reg = dframe[dframe['Continent'] == 'North_America']
...
## Grab Data ## 
    					...
    					dframe_cold_season = dframe_reg[dframe_reg['Season'] == 'Cold']
...
## Grab Data ## 
    					...
    					dframe_warm_season = dframe_reg[dframe_reg['Season'] == 'Warm']
...
## Calculate Average Metrics Across Grid Cells ##
    					...
    					n,bins,patches = plt.hist(x=JRA_bias_warm_master, bins='auto')
    					plt.show()
    					...
    					sample_size = len(naive_bias_warm_master)
    					print('Sample Size for'+str(lyr_l)+':',str(sample_size))
...
## Create Dataframe ##
					...
					index=["Naive Blend","Naive Blend no JRA55 Old","Naive Blend All","Naive Blend no JRA55","CFSR","ERA-Interim","ERA5","ERA5-Land","JRA55","MERRA2","GLDAS-Noah","GLDAS-CLSM"])}
    					...
    					metrics_all_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blended_metrics/cold_warm_season/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_'+str(lyr_l)+'_naive_metrics_CLSM_'+str(permafrost_type_o)+'_grid_avg_BEST_Sep2021_NAm.csv'])  					
