    		...
    		for l in lyr:
    			...
    			for m in thr:
    				...
    				for n in permafrost_type:
    					permafrost_type_o = n
    					cold_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/sites_1yr/'+str(remap_type)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_scatterplot_CMOS_CLSM_subset_permafrost_cold_warm_BEST_Sep2021_NWT_Yukon_sites_1yr.csv'])
    					warm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/sites_1yr/'+str(remap_type)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_scatterplot_CMOS_CLSM_subset_permafrost_cold_warm_BEST_Sep2021_NWT_Yukon_sites_1yr.csv'])    			
    					...
    					dframe_continent = dframe[dframe['Continent'] == 'Eurasia']
    					dframe_cold = dframe_continent[dframe_continent['Season'] == 'Cold']
    					dframe_warm = dframe_continent[dframe_continent['Season'] == 'Warm']
...
# Calculate grid cell level statistics #
    					...
    					for p in gcell_cold_uq: #loop through grid cells
    						...
    						print(dframe_cold_season_gcell)

    						if (len(dframe_cold_season_gcell) < 2):
    							continue
    							print("skipped Grid Cell:",gcell_p," As N = 1")
...
##################### Create Taylor Diagrams ####################
    					...
    					taylor_plt_fil_cold = ''.join([taylor_dir,str(lyr_l)+'_taylor_diagram_cold_'+str(permafrost_type_o)+'_combined_grid_cell_BEST_Sep2021_NWT_Yukon_sites_1yr_Eur.png'])
...
##################### Create Taylor Diagrams ####################
    					...
    					taylor_dir = '/mnt/data/users/herringtont/soil_temp/plots/taylor_diagrams/new_data/CMOS_poster/CLSM_res/'
    					taylor_plt_fil_warm = ''.join([taylor_dir,str(lyr_l)+'_taylor_diagram_warm_'+str(permafrost_type_o)+'_combined_grid_cell_BEST_Sep2021_NWT_Yukon_sites_1yr_Eur.png'])
