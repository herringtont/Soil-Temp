...
# North America #
    					...
    					dframe_cold_season_NAm = dframe_NAm[dframe_NAm['Season'] == 'Cold']

    					dframe_cold_season_NAm_permafrost = dframe_cold_season_NAm[(dframe_cold_season_NAm['RS 2002 Permafrost'] == 'continuous') | (dframe_cold_season_NAm['RS 2002 Permafrost'] == 'discontinuous')]

    					gcell_cold_NAm = dframe_cold_season_NAm_permafrost['Grid Cell'].values
    					gcell_cold_uq_NAm = np.unique(gcell_cold_NAm)


    					dframe_warm_season_NAm = dframe_NAm[dframe_NAm['Season'] == 'Warm']

    					dframe_warm_season_NAm_permafrost = dframe_warm_season_NAm[(dframe_warm_season_NAm['RS 2002 Permafrost'] == 'continuous') | (dframe_warm_season_NAm['RS 2002 Permafrost'] == 'discontinuous')]

    					gcell_warm_NAm = dframe_warm_season_NAm_permafrost['Grid Cell'].values
    					gcell_warm_uq_NAm = np.unique(gcell_warm_NAm)



## cold season ##
    					for p in gcell_cold_uq_NAm: # loop through grid cells
    						gcell_p = p
    						if (gcell_p == 33777):
    							continue
    						dframe_cold_season_gcell = dframe_cold_season_NAm_permafrost[dframe_cold_season_NAm_permafrost['Grid Cell'] == gcell_p]
...
## warm season ##
    					...
    					for p in gcell_warm_uq_NAm: # loop through grid cells
    						gcell_p = p
    						if (gcell_p == 33777):
    							continue
    						dframe_warm_season_gcell = dframe_warm_season_NAm_permafrost[dframe_warm_season_NAm_permafrost['Grid Cell'] == gcell_p]
...
# Eurasia #
    					...
    					dframe_cold_season_Eur = dframe_Eur[dframe_Eur['Season'] == 'Cold']

    					dframe_cold_season_Eur_permafrost = dframe_cold_season_Eur[(dframe_cold_season_Eur['RS 2002 Permafrost'] == 'continuous') | (dframe_cold_season_Eur['RS 2002 Permafrost'] == 'discontinuous')]

    					gcell_cold_Eur = dframe_cold_season_Eur_permafrost['Grid Cell'].values
    					gcell_cold_uq_Eur = np.unique(gcell_cold_Eur)


    					dframe_warm_season_Eur = dframe_Eur[dframe_Eur['Season'] == 'Warm']

    					dframe_warm_season_Eur_permafrost = dframe_warm_season_Eur[(dframe_warm_season_Eur['RS 2002 Permafrost'] == 'continuous') | (dframe_warm_season_Eur['RS 2002 Permafrost'] == 'discontinuous')]

    					gcell_warm_Eur = dframe_warm_season_Eur_permafrost['Grid Cell'].values
    					gcell_warm_uq_Eur = np.unique(gcell_warm_Eur)




## cold season ##
    					for p in gcell_cold_uq_Eur: # loop through grid cells
    						gcell_p = p
    						if (gcell_p == 33777):
    							continue
    						dframe_cold_season_gcell = dframe_cold_season_Eur_permafrost[dframe_cold_season_Eur_permafrost['Grid Cell'] == gcell_p]
    						if (len(dframe_cold_season_gcell) < 2):
    							continue    						
    						station_temp_cold = dframe_cold_season_gcell['Station'].values
    						naive_temp_cold = dframe_cold_season_gcell['Naive Blend'].values
...
## warm season ##
    					...
    					for p in gcell_warm_uq_Eur: # loop through grid cells
    						gcell_p = p
    						if (gcell_p == 33777):
    							continue
    						dframe_warm_season_gcell = dframe_warm_season_Eur_permafrost[dframe_warm_season_Eur_permafrost['Grid Cell'] == gcell_p]
    						if (len(dframe_warm_season_gcell) < 2):
    							continue    						
    						station_temp_warm = dframe_warm_season_gcell['Station'].values
    						naive_temp_warm = dframe_warm_season_gcell['Naive Blend'].values
...
## Bias ##
    					...
    					diff_bias_ensmean = mean(naive_all_bias_warm_master_NAm) - mean(naive_all_bias_warm_master_Eur)
    					diff_bias_CFSR = mean(CFSR_bias_warm_master_NAm) - mean(CFSR_bias_warm_master_Eur)
    					diff_bias_ERAI = mean(ERAI_bias_warm_master_NAm) - mean(ERAI_bias_warm_master_Eur)
    					diff_bias_ERA5 = mean(ERA5_bias_warm_master_NAm) - mean(ERA5_bias_warm_master_Eur)
    					diff_bias_ERA5_Land = mean(ERA5_Land_bias_warm_master_NAm) - mean(ERA5_Land_bias_warm_master_Eur)
    					diff_bias_JRA = mean(JRA_bias_warm_master_NAm) - mean(JRA_bias_warm_master_Eur)
    					diff_bias_MERRA2 = mean(MERRA2_bias_warm_master_NAm) - mean(MERRA2_bias_warm_master_Eur)
    					diff_bias_GLDAS = mean(GLDAS_bias_warm_master_NAm) - mean(GLDAS_bias_warm_master_Eur)
    					diff_bias_GLDAS_CLSM = mean(GLDAS_CLSM_bias_warm_master_NAm) - mean(GLDAS_CLSM_bias_warm_master_Eur)


    					bias_array_warm_NAm = [mean(naive_all_bias_warm_master_NAm),mean(CFSR_bias_warm_master_NAm),mean(ERAI_bias_warm_master_NAm),mean(ERA5_bias_warm_master_NAm),mean(ERA5_Land_bias_warm_master_NAm),mean(JRA_bias_warm_master_NAm),mean(MERRA2_bias_warm_master_NAm),mean(GLDAS_bias_warm_master_NAm),mean(GLDAS_CLSM_bias_warm_master_NAm)]
    					bias_array_warm_Eur = [mean(naive_all_bias_warm_master_Eur),mean(CFSR_bias_warm_master_Eur),mean(ERAI_bias_warm_master_Eur),mean(ERA5_bias_warm_master_Eur),mean(ERA5_Land_bias_warm_master_Eur),mean(JRA_bias_warm_master_Eur),mean(MERRA2_bias_warm_master_Eur),mean(GLDAS_bias_warm_master_Eur),mean(GLDAS_CLSM_bias_warm_master_Eur)]

    					bias_diff_warm = [diff_bias_ensmean,diff_bias_CFSR,diff_bias_ERAI,diff_bias_ERA5,diff_bias_ERA5_Land,diff_bias_JRA,diff_bias_MERRA2,diff_bias_GLDAS,diff_bias_GLDAS_CLSM]

    					w,p = scipy.stats.wilcoxon(bias_diff_warm)
    					print(w,p)
...
#    					metrics_all_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blended_metrics/cold_warm_season/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_'+str(lyr_l)+'_naive_metrics_CLSM_'+str(permafrost_type_o)+'_grid_avg_BEST_Sep2021_newJRA.csv'])  					
