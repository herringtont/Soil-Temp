    	...
    	for k in olr:
    		...
    		for l in lyr:
    			...
    			for m in thr:
    				...
    				station_scatter = dframe_scatter['BEST Air'].values
    				naive_all_scatter = dframe_scatter['Naive Blend All Air'].values
    				CFSR_scatter = dframe_scatter['CFSR Air'].values
    				ERAI_scatter = dframe_scatter['ERA-Interim Air'].values
    				ERA5_scatter = dframe_scatter['ERA5 Air'].values
    				ERA5_Land_scatter = dframe_scatter['ERA5-Land Air'].values
    				JRA_scatter = dframe_scatter['JRA55 Air'].values
    				MERRA2_scatter = dframe_scatter['MERRA2 Air'].values
    				GLDAS_scatter = dframe_scatter['GLDAS-Noah Air'].values
    				GLDAS_CLSM_scatter = dframe_scatter['GLDAS-CLSM Air'].values
...
############ create temperature arrays ################
    				...
    				stdev_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_stdev_temp/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_stdev_temp_4_degree_bins_CMOS_CLSM_noGreenland_BEST_Sep2021_airtemp.csv'])
...
############## Create Figure ##############
    fig = plt.figure()
    fig,axs=plt.subplots(nrows=1,ncols=1,sharex='all',figsize=(20,20))

    matplotlib.rc('xtick', labelsize=25) 
    matplotlib.rc('ytick', labelsize=25)

    top_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_stdev_temp/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_simple_average_zscore_top_30cm_thr_100_stdev_temp_4_degree_bins_CMOS_CLSM_noGreenland_BEST_Sep2021_airtemp.csv'])   				
    #btm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_stdev_temp/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_simple_average_zscore_30cm_300cm_thr_100_stdev_temp_4_degree_bins_CMOS_CLSM_noGreenland_BEST_Sep2021_airtemp.csv'])
    ...
    print(top_ERA5_stdev)
    print(top_ERA5_Land_stdev)
    ...
    ax1.set_yticks([0,0.25,0.5,0.75,1,1.25,1.5,1.75,2,2.25,2.5,2.75,3,3.25,3.5,3.75,4])
    lines = []
    labels = []
    ...
    plt.xlabel('BEST Air Temperature($^\circ$ C)', fontweight='bold',fontsize=25)
    plt.ylabel('Reanalysis Air Temp Std Dev($^\circ$ C)',fontweight='bold', fontsize=25)   
    ...
    fig_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/plots/naive_blend_stdev_plots/new_data/CMOS_poster/CLSM_res/'+str(remap_type)+'_all_layers_thr_100_stdev_temp_4_degree_bins_CMOS_2panel_CLSM_noGreenland_BEST_Sep2021_airtemp_newJRA.png'])
    plt.tight_layout()
    plt.savefig(fig_fil)
    plt.close()
