...
cold_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_timeseries/Sep2021/remapcon_dframe_timeseries_BEST_-2C_CMOS_CLSM_subset_permafrost_Sep2021_cold.csv'])
warm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_timeseries/Sep2021/remapcon_dframe_timeseries_BEST_-2C_CMOS_CLSM_subset_permafrost_Sep2021_warm.csv'])

dframe_cold = pd.read_csv(cold_fil)
dframe_warm = pd.read_csv(warm_fil)


date_warm = dframe_warm['Date']
date_cold = dframe_cold['Date']

dates_warm = date_warm.astype(str)
datetime_warm = pd.to_datetime(dates_warm)

dframe_warm['Datetime'] = datetime_warm
dframe_warm = dframe_warm.set_index(pd.DatetimeIndex(dframe_warm['Datetime']))



dates_cold = date_cold.astype(str)
datetime_cold = pd.to_datetime(dates_cold)

dframe_cold['Datetime'] = datetime_cold
dframe_cold = dframe_cold.set_index(pd.DatetimeIndex(dframe_cold['Datetime']))

#### loop through grid cells ####

gcells_cold = dframe_cold['Grid Cell'].values
gcells_warm = dframe_warm['Grid Cell'].values

gcells_all = []
gcells_all.append(gcells_cold)
gcells_all.append(gcells_warm)
gcells_all = [i for sub in gcells_all for i in sub]
...
for i in gcells_uq:   #loop through grid cells and grab grid cell level data
    ...
    gcell_i = i
    dframe_gcell_cold = dframe_cold[dframe_cold['Grid Cell'] == gcell_i]
    dframe_gcell_warm = dframe_warm[dframe_warm['Grid Cell'] == gcell_i]
    lat_warm = dframe_gcell_warm['Lat'].values
    lat_cold = dframe_gcell_cold['Lat'].values
    lon_warm = dframe_gcell_warm['Lon'].values
    lon_cold = dframe_gcell_cold['Lon'].values

    len_lat_cold = len(lat_cold)
    ...
    if(len_lat_cold > 0):
    	...
    	lat_fig = lat_warm[0]
    	lon_fig = lon_warm[0]
    permafrost_type_cold = dframe_gcell_cold['Permafrost Type'].values
    permafrost_type_warm = dframe_gcell_warm['Permafrost Type'].values
    permafrost_type_cold = permafrost_type_cold[0]
    permafrost_type_warm = permafrost_type_warm[0]

    len_permafrost_cold = len(permafrost_type_cold)
    len_permafrost_warm = len(permafrost_type_warm)

    if (permafrost_type_cold == 'continuous' or permafrost_type_cold == 'discontinuous'):
    	permafrost_cold = 'permafrost'

    elif (permafrost_type_cold == 'none'):
    	permafrost_cold = 'none'

    else:
    	permafrost_cold = 'NaN'

    if (permafrost_type_warm == 'continuous' or permafrost_type_warm == 'discontinuous'):
    	permafrost_warm = 'permafrost'

    elif (permafrost_type_warm == 'none'):
    	permafrost_warm = 'none'
    else:
    	permafrost_warm = 'NaN'


    if (permafrost_cold == 'NaN'):
    	permafrost_type = permafrost_warm

    else:
    	permafrost_type = permafrost_cold

    dframe_gcell_cold_top = dframe_gcell_cold[dframe_gcell_cold['Layer'] == 'top_30cm']
    dframe_gcell_cold_btm = dframe_gcell_cold[dframe_gcell_cold['Layer'] == '30cm_300cm']
    dframe_gcell_warm_top = dframe_gcell_warm[dframe_gcell_warm['Layer'] == 'top_30cm']
    dframe_gcell_warm_btm = dframe_gcell_warm[dframe_gcell_warm['Layer'] == '30cm_300cm']

    dframe_gcell_cold_top = dframe_gcell_cold_top.resample('M').first().fillna(value=np.nan)
    dframe_gcell_cold_btm = dframe_gcell_cold_btm.resample('M').first().fillna(value=np.nan)
    dframe_gcell_warm_top = dframe_gcell_warm_top.resample('M').first().fillna(value=np.nan)
    dframe_gcell_warm_btm = dframe_gcell_warm_btm.resample('M').first().fillna(value=np.nan)

    dates_cold_top = dframe_gcell_cold_top['Date'].values
    dates_cold_btm = dframe_gcell_cold_btm['Date'].values
    dates_warm_top = dframe_gcell_warm_top['Date'].values
    dates_warm_btm = dframe_gcell_warm_btm['Date'].values
    
    station_cold_top = dframe_gcell_cold_top['Station'].values
    station_cold_btm = dframe_gcell_cold_btm['Station'].values
    station_warm_top = dframe_gcell_warm_top['Station'].values
    station_warm_btm = dframe_gcell_warm_btm['Station'].values

    naive_all_cold_top = dframe_gcell_cold_top['Ensemble Mean'].values
    naive_all_cold_btm = dframe_gcell_cold_btm['Ensemble Mean'].values
    naive_all_warm_top = dframe_gcell_warm_top['Ensemble Mean'].values
    naive_all_warm_btm = dframe_gcell_warm_btm['Ensemble Mean'].values

    CFSR_cold_top = dframe_gcell_cold_top['CFSR'].values
    CFSR_cold_btm = dframe_gcell_cold_btm['CFSR'].values
    CFSR_warm_top = dframe_gcell_warm_top['CFSR'].values
    CFSR_warm_btm = dframe_gcell_warm_btm['CFSR'].values

    ERAI_cold_top = dframe_gcell_cold_top['ERA-Interim'].values
    ERAI_cold_btm = dframe_gcell_cold_btm['ERA-Interim'].values
    ERAI_warm_top = dframe_gcell_warm_top['ERA-Interim'].values
    ERAI_warm_btm = dframe_gcell_warm_btm['ERA-Interim'].values

    ERA5_cold_top = dframe_gcell_cold_top['ERA5'].values
    ERA5_cold_btm = dframe_gcell_cold_btm['ERA5'].values
    ERA5_warm_top = dframe_gcell_warm_top['ERA5'].values
    ERA5_warm_btm = dframe_gcell_warm_btm['ERA5'].values

    ERA5_Land_cold_top = dframe_gcell_cold_top['ERA5-Land'].values
    ERA5_Land_cold_btm = dframe_gcell_cold_btm['ERA5-Land'].values
    ERA5_Land_warm_top = dframe_gcell_warm_top['ERA5-Land'].values
    ERA5_Land_warm_btm = dframe_gcell_warm_btm['ERA5-Land'].values

    JRA_cold_top = dframe_gcell_cold_top['JRA55'].values
    JRA_cold_btm = dframe_gcell_cold_btm['JRA55'].values
    JRA_warm_top = dframe_gcell_warm_top['JRA55'].values
    JRA_warm_btm = dframe_gcell_warm_btm['JRA55'].values

    MERRA2_cold_top = dframe_gcell_cold_top['MERRA2'].values
    MERRA2_cold_btm = dframe_gcell_cold_btm['MERRA2'].values
    MERRA2_warm_top = dframe_gcell_warm_top['MERRA2'].values
    MERRA2_warm_btm = dframe_gcell_warm_btm['MERRA2'].values

    GLDAS_cold_top = dframe_gcell_cold_top['GLDAS-Noah'].values
    GLDAS_cold_btm = dframe_gcell_cold_btm['GLDAS-Noah'].values
    GLDAS_warm_top = dframe_gcell_warm_top['GLDAS-Noah'].values
    GLDAS_warm_btm = dframe_gcell_warm_btm['GLDAS-Noah'].values

    GLDAS_CLSM_cold_top = dframe_gcell_cold_top['GLDAS-CLSM'].values
    GLDAS_CLSM_cold_btm = dframe_gcell_cold_btm['GLDAS-CLSM'].values
    GLDAS_CLSM_warm_top = dframe_gcell_warm_top['GLDAS-CLSM'].values
    GLDAS_CLSM_warm_btm = dframe_gcell_warm_btm['GLDAS-CLSM'].values


## grab dates and create date range that covers all dates across top/btm layers
    dates_all = []
    dates_all.append(dates_cold_top)
    dates_all.append(dates_cold_btm)
    dates_all.append(dates_warm_top)
    dates_all.append(dates_warm_btm)

    dates_cold_top_str = dates_cold_top.astype(str)
    datetime_cold_top = pd.to_datetime(dates_cold_top_str)

    dates_cold_btm_str = dates_cold_btm.astype(str)
    datetime_cold_btm = pd.to_datetime(dates_cold_btm_str)

    dates_warm_top_str = dates_warm_top.astype(str)
    datetime_warm_top = pd.to_datetime(dates_warm_top_str)

    dates_warm_btm_str = dates_warm_btm.astype(str)
    datetime_warm_btm = pd.to_datetime(dates_warm_btm_str)

    dates_all = [i for sub in dates_all for i in sub]
    dates_uq = np.unique(dates_all)
    #print(dates_uq)
...
## create 2x2 figure with cold season on left, warm season on right; near surface on top and depth at bottom ##
    ...
    fig,axs=plt.subplots(nrows=2,ncols=2,sharex='all',figsize=(20,20))
    ...
    ax1 = plt.subplot(2,2,1) #near surface, cold season
    ax1.plot(datetime_cold_top,station_cold_top,label='Station',marker='p',markerfacecolor='violet',markersize=10.0,color='violet',linewidth=10.0)
    ax1.plot(datetime_cold_top,naive_all_cold_top,label='Ensemble Mean',marker='p',markerfacecolor='hotpink',markersize=8.0,color='hotpink',linewidth=8.0)
    ax1.plot(datetime_cold_top,CFSR_cold_top,label='CFSR',marker='s',markerfacecolor='darkviolet',markersize=3.0,color='darkviolet',linewidth=3.0)
    ax1.plot(datetime_cold_top,ERAI_cold_top,label="ERA-Interim",marker='v',markerfacecolor='springgreen',markersize=3.0,color='springgreen',linewidth=3.0,linestyle='dotted')
    ax1.plot(datetime_cold_top,ERA5_cold_top,label="ERA5",marker='^',markerfacecolor='greenyellow',markersize=3.0,color='greenyellow',linewidth=3.0,linestyle='dashed')
    ax1.plot(datetime_cold_top,ERA5_Land_cold_top,label="ERA5-Land",marker='^',markerfacecolor='dodgerblue',markersize=3.0,color='dodgerblue',linewidth=3.0)
    ax1.plot(datetime_cold_top,JRA_cold_top,label="JRA-55",marker='*',markerfacecolor='red',markersize=3.0,color='red',linewidth=3.0)
    ax1.plot(datetime_cold_top,MERRA2_cold_top,label="MERRA2",marker='D',markerfacecolor='goldenrod',markersize=3.0,color='goldenrod',linewidth=3.0)
    ax1.plot(datetime_cold_top,GLDAS_cold_top,label="GLDAS-Noah",marker='x',markerfacecolor='black',markersize=3.0,color='black',linewidth=3.0)
    ax1.plot(datetime_cold_top,GLDAS_CLSM_cold_top,label="GLDAS-CLSM",marker='x',markerfacecolor='darkgrey',markersize=3.0,color='darkgrey',linewidth=3.0)
    ax1.set(xlim=[min_date,max_date])


    handles1 = [mpl_patches.Rectangle((0, 0), 1, 1, fc="white", ec="white", lw=0, alpha=0)]
    ax1text = []
    ax1text.append('Near Surface, Cold Season')
    ax1.legend(handles1, ax1text, loc='best', fontsize=20, fancybox=False, framealpha=0, handlelength=0, handletextpad=0)
    ...
    ax2 = plt.subplot(2,2,2) #near surface, warm season
    ax2.plot(datetime_warm_top,station_warm_top,label='Station',marker='p',markerfacecolor='violet',markersize=10.0,color='violet',linewidth=10.0)
    ax2.plot(datetime_warm_top,naive_all_warm_top,label='Ensemble Mean',marker='p',markerfacecolor='hotpink',markersize=8.0,color='hotpink',linewidth=8.0)
    ax2.plot(datetime_warm_top,CFSR_warm_top,label='CFSR',marker='s',markerfacecolor='darkviolet',markersize=3.0,color='darkviolet',linewidth=3.0)
    ax2.plot(datetime_warm_top,ERAI_warm_top,label="ERA-Interim",marker='v',markerfacecolor='springgreen',markersize=3.0,color='springgreen',linewidth=3.0,linestyle='dotted')
    ax2.plot(datetime_warm_top,ERA5_warm_top,label="ERA5",marker='^',markerfacecolor='greenyellow',markersize=3.0,color='greenyellow',linewidth=3.0,linestyle='dashed')
    ax2.plot(datetime_warm_top,ERA5_Land_warm_top,label="ERA5-Land",marker='^',markerfacecolor='dodgerblue',markersize=3.0,color='dodgerblue',linewidth=3.0)
    ax2.plot(datetime_warm_top,JRA_warm_top,label="JRA-55",marker='*',markerfacecolor='red',markersize=3.0,color='red',linewidth=3.0)
    ax2.plot(datetime_warm_top,MERRA2_warm_top,label="MERRA2",marker='D',markerfacecolor='goldenrod',markersize=3.0,color='goldenrod',linewidth=3.0)
    ax2.plot(datetime_warm_top,GLDAS_warm_top,label="GLDAS-Noah",marker='x',markerfacecolor='black',markersize=3.0,color='black',linewidth=3.0)
    ax2.plot(datetime_warm_top,GLDAS_CLSM_warm_top,label="GLDAS-CLSM",marker='x',markerfacecolor='darkgrey',markersize=3.0,color='darkgrey',linewidth=3.0)
    ...
    ax2text = []
    ax2text.append('Near Surface, Warm Season')
    ax2.legend(handles2, ax2text, loc='best', fontsize=20, fancybox=False, framealpha=0, handlelength=0, handletextpad=0)
    ...
    ax3 = plt.subplot(2,2,3) #depth, cold season
    ax3.plot(datetime_cold_btm,station_cold_btm,label='Station',marker='p',markerfacecolor='violet',markersize=10.0,color='violet',linewidth=10.0)
    ax3.plot(datetime_cold_btm,naive_all_cold_btm,label='Ensemble Mean',marker='p',markerfacecolor='hotpink',markersize=8.0,color='hotpink',linewidth=8.0)
    ax3.plot(datetime_cold_btm,CFSR_cold_btm,label='CFSR',marker='s',markerfacecolor='darkviolet',markersize=3.0,color='darkviolet',linewidth=3.0)
    ax3.plot(datetime_cold_btm,ERAI_cold_btm,label="ERA-Interim",marker='v',markerfacecolor='springgreen',markersize=3.0,color='springgreen',linewidth=3.0,linestyle='dotted')
    ax3.plot(datetime_cold_btm,ERA5_cold_btm,label="ERA5",marker='^',markerfacecolor='greenyellow',markersize=3.0,color='greenyellow',linewidth=3.0,linestyle='dashed')
    ax3.plot(datetime_cold_btm,ERA5_Land_cold_btm,label="ERA5-Land",marker='^',markerfacecolor='dodgerblue',markersize=3.0,color='dodgerblue',linewidth=3.0)
    ax3.plot(datetime_cold_btm,JRA_cold_btm,label="JRA-55",marker='*',markerfacecolor='red',markersize=3.0,color='red',linewidth=3.0)
    ax3.plot(datetime_cold_btm,MERRA2_cold_btm,label="MERRA2",marker='D',markerfacecolor='goldenrod',markersize=3.0,color='goldenrod',linewidth=3.0)
    ax3.plot(datetime_cold_btm,GLDAS_cold_btm,label="GLDAS-Noah",marker='x',markerfacecolor='black',markersize=3.0,color='black',linewidth=3.0)
    ax3.plot(datetime_cold_btm,GLDAS_CLSM_cold_btm,label="GLDAS-CLSM",marker='x',markerfacecolor='darkgrey',markersize=3.0,color='darkgrey',linewidth=3.0)      
    ...
    ax3.legend(handles3, ax3text, loc='best', fontsize=20, fancybox=False, framealpha=0, handlelength=0, handletextpad=0)
    ...
    ax4 = plt.subplot(2,2,4) #depth, cold season
    ax4.plot(datetime_warm_btm,station_warm_btm,label='Station',marker='p',markerfacecolor='violet',markersize=10.0,color='violet',linewidth=10.0)
    ax4.plot(datetime_warm_btm,naive_all_warm_btm,label='Ensemble Mean',marker='p',markerfacecolor='hotpink',markersize=8.0,color='hotpink',linewidth=8.0)
    ax4.plot(datetime_warm_btm,CFSR_warm_btm,label='CFSR',marker='s',markerfacecolor='darkviolet',markersize=3.0,color='darkviolet',linewidth=3.0)
    ax4.plot(datetime_warm_btm,ERAI_warm_btm,label="ERA-Interim",marker='v',markerfacecolor='springgreen',markersize=3.0,color='springgreen',linewidth=3.0,linestyle='dotted')
    ax4.plot(datetime_warm_btm,ERA5_warm_btm,label="ERA5",marker='^',markerfacecolor='greenyellow',markersize=3.0,color='greenyellow',linewidth=3.0,linestyle='dashed')
    ax4.plot(datetime_warm_btm,ERA5_Land_warm_btm,label="ERA5-Land",marker='^',markerfacecolor='dodgerblue',markersize=3.0,color='dodgerblue',linewidth=3.0)
    ax4.plot(datetime_warm_btm,JRA_warm_btm,label="JRA-55",marker='*',markerfacecolor='red',markersize=3.0,color='red',linewidth=3.0)
    ax4.plot(datetime_warm_btm,MERRA2_warm_btm,label="MERRA2",marker='D',markerfacecolor='goldenrod',markersize=3.0,color='goldenrod',linewidth=3.0)
    ax4.plot(datetime_warm_btm,GLDAS_warm_btm,label="GLDAS-Noah",marker='x',markerfacecolor='black',markersize=3.0,color='black',linewidth=3.0)
    ax4.plot(datetime_warm_btm,GLDAS_CLSM_warm_btm,label="GLDAS-CLSM",marker='x',markerfacecolor='darkgrey',markersize=3.0,color='darkgrey',linewidth=3.0)
    ax4.set(xlim=[min_date,max_date])

    handles4 = [mpl_patches.Rectangle((0, 0), 1, 1, fc="white", ec="white", lw=0, alpha=0)]
    ax4text = []
    ax4text.append('Depth, Warm Season')
    ax4.legend(handles4, ax4text, loc='best', fontsize=20, fancybox=False, framealpha=0, handlelength=0, handletextpad=0)
    ...
    fig_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/plots/naive_blend_timeseries/Sep2021/'+str(permafrost_type)+'/'+str(remap_type)+'_all_layers_grid_cell_'+str(gcell_i)+'.png'])
    plt.tight_layout()




    at = AnchoredText("lat($^\circ$ N):"+str(lat_fig)+" , lon($^\circ$ E):"+str(lon_fig), prop=dict(size=20), frameon=False, loc='upper left')
