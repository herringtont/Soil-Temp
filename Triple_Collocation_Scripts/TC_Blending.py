...
######################## Loop Through Products to extract the product-specific values #######################
    			...
    			for name in datasets:
    				...
    				if name == "CFSR":
    					...
    					CFSR_scale_factor_mean = np.mean(Scale_Factor)
    				...
    				if name == "ERA-Interim":
    					...
    					ERAI_scale_factor_mean = np.mean(Scale_Factor)
    				...
    				if name == "ERA5":
    					...
    					ERA5_scale_factor_mean = np.mean(Scale_Factor)
    				...
    				if name == "JRA-55":
    					...
    					JRA_scale_factor_mean = np.mean(Scale_Factor)
    				...
    				if name == "MERRA2":
    					...
    					MERRA2_scale_factor_mean = np.mean(Scale_Factor)
    				...
    				if name == "GLDAS":
    					...
    					GLDAS_scale_factor_mean = np.mean(Scale_Factor)


####################### Scale individual anomaly datasets ############

    			CFSR_scaled_anom = np.array(CFSR_master_all_1D)*CFSR_scale_factor_mean
    			ERAI_scaled_anom = np.array(ERAI_master_all_1D)*ERAI_scale_factor_mean
    			ERA5_scaled_anom = np.array(ERA5_master_all_1D)*ERA5_scale_factor_mean
    			JRA_scaled_anom = np.array(JRA_master_all_1D)*JRA_scale_factor_mean    			
    			MERRA2_scaled_anom = np.array(MERRA2_master_all_1D)*MERRA2_scale_factor_mean
    			GLDAS_scaled_anom = np.array(GLDAS_master_all_1D)*GLDAS_scale_factor_mean
			
###################### Scale Climatologies and convert to scaled raw data ################

### In order to convert back to absolute temperatures, we will add back the climatology*scale factor

    			CFSR_scaled_clim = np.array(CFSR_clim_master_all_1D)*CFSR_scale_factor_mean
    			ERAI_scaled_clim = np.array(ERAI_clim_master_all_1D)*ERAI_scale_factor_mean
    			ERA5_scaled_clim = np.array(ERA5_clim_master_all_1D)*ERA5_scale_factor_mean
    			JRA_scaled_clim = np.array(JRA_clim_master_all_1D)*JRA_scale_factor_mean
    			MERRA2_scaled_clim = np.array(MERRA2_clim_master_all_1D)*MERRA2_scale_factor_mean
    			GLDAS_scaled_clim = np.array(GLDAS_clim_master_all_1D)*GLDAS_scale_factor_mean

##### Convert back into absolute temperatures #####

    			CFSR_scaled_raw = CFSR_scaled_anom + CFSR_scaled_clim
    			ERAI_scaled_raw = ERAI_scaled_anom + ERAI_scaled_clim
    			ERA5_scaled_raw = ERA5_scaled_anom + ERA5_scaled_clim
    			JRA_scaled_raw = JRA_scaled_anom + JRA_scaled_clim
    			MERRA2_scaled_raw = MERRA2_scaled_anom + MERRA2_scaled_clim
    			GLDAS_scaled_raw = GLDAS_scaled_anom + GLDAS_scaled_clim


##################### Calculate weighted averages and create blended product ###############

#### calculate product weights based on fMSE values ####

    			wght_denom = (1/CFSR_fMSE_mean)+(1/ERAI_fMSE_mean)+(1/ERA5_fMSE_mean)+(1/JRA_fMSE_mean)+(1/MERRA2_fMSE_mean)+(1/GLDAS_fMSE_mean)
    			CFSR_wght = (1/CFSR_fMSE_mean)/wght_denom
    			ERAI_wght = (1/ERAI_fMSE_mean)/wght_denom
    			ERA5_wght = (1/ERA5_fMSE_mean)/wght_denom
    			JRA_wght = (1/JRA_fMSE_mean)/wght_denom
    			MERRA2_wght = (1/MERRA2_fMSE_mean)/wght_denom
    			GLDAS_wght = (1/GLDAS_fMSE_mean)/wght_denom

    			print('Scaling Factors:')
    			print('CFSR:',CFSR_scale_factor_mean)
    			print('ERAI:',ERAI_scale_factor_mean)
    			print('ERA5:',ERA5_scale_factor_mean)
    			print('JRA55:',JRA_scale_factor_mean)
    			print('MERRA2:',MERRA2_scale_factor_mean)
    			print('GLDAS:',GLDAS_scale_factor_mean)

    			print('Product Weights:')
    			print('CFSR:',CFSR_wght)
    			print('ERAI:',ERAI_wght)
    			print('ERA5:',ERA5_wght)
    			print('JRA55:',JRA_wght)
    			print('MERRA2:',MERRA2_wght)
    			print('GLDAS:',GLDAS_wght)

#### create blended product ####

    			blended_anom = (CFSR_wght*CFSR_scaled_anom)+(ERAI_wght*ERAI_scaled_anom)+(ERA5_wght*ERA5_scaled_anom)+(JRA_wght*JRA_scaled_anom)+(MERRA2_wght*MERRA2_scaled_anom)+(GLDAS_wght*GLDAS_scaled_anom)
    			blended_raw = (CFSR_wght*CFSR_scaled_raw)+(ERAI_wght*ERAI_scaled_raw)+(ERA5_wght*ERA5_scaled_raw)+(JRA_wght*JRA_scaled_raw)+(MERRA2_wght*MERRA2_scaled_raw)+(GLDAS_wght*GLDAS_scaled_raw)


#### create finalized blended dataframes ####

    			dframe_blended_raw = pd.DataFrame(data=date_master_all_1D,columns=['Date'])
    			dframe_blended_raw['Grid Cell'] = grid_master_all_1D
    			
    			dframe_blended_anom = pd.DataFrame(data=date_master_all_1D,columns=['Date'])
    			dframe_blended_anom['Grid Cell'] = grid_master_all_1D
    			...
    			dframe_blended_raw['TC Blended'] = blended_raw
    			dframe_blended_raw['In-Situ'] = station_master_raw_1D
    			dframe_blended_raw['CFSR'] = CFSR_master_raw_1D
    			...
    			dframe_blended_anom['TC Blended'] = blended_anom
    			dframe_blended_anom['In-Situ'] = station_master_all_1D
    			dframe_blended_anom['CFSR'] = CFSR_master_all_1D
    			...
    			blend_fil_anom = ''.join(['/mnt/data/users/herringtont/soil_temp/Blended_Product/collocated/TC_blended/anom/'+str(remap_type)+'_'+str(olri)+'_'+str(lyrj)+'_thr'+str(thrk)+'_TC_blended_anom.csv'])
    			blend_fil_raw = ''.join(['/mnt/data/users/herringtont/soil_temp/Blended_Product/collocated/TC_blended/raw/'+str(remap_type)+'_'+str(olri)+'_'+str(lyrj)+'_thr'+str(thrk)+'_TC_blended.csv'])
