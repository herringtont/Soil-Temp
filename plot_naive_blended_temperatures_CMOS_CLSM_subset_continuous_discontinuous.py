...
for i in rmp_type:
    ...
    rnys_dir = ''.join(['/mnt/data/users/herringtont/soil_temp/reanalysis/monthly/remap/rename/obs_depths/common_grid_CLSM/'+str(remap_type)+'/common_date/'])

    air_dir = ''.join(['/mnt/data/users/herringtont/soil_temp/reanalysis/2m_AirTemp/rename/land_only/common_grid_CLSM/'+str(remap_type)+'/'])
    ...
    ERA5_MAAT_fi = ''.join([air_dir,'MAAT/ERA5_2m_air_MAAT_1981_2010_clim.nc'])
    ...
    for j in naive_type:
    	naive_type_j = j
    	naive_dir_raw = ''.join(['/mnt/data/users/herringtont/soil_temp/reanalysis/monthly/remap/rename/obs_depths/common_grid_CLSM/'+str(remap_type)+'/common_date/'])
    	...
    	for k in olr:
    		...
    		for l in lyr:
    			...
    			for m in thr:
    				thr_m = m
    				insitu_dir =  ''.join(['/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_anom/'+str(remap_type)+'/no_outliers/'+str(olr_k)+'/'+str(in_situ_layer)+'/thr_'+str(thr_m)+'/CLSM/'])
##### Create Master Arrays #####
    				...
    				continent_master = []
    				RS_2002_permafrost_master = []
    				Brown_1970_permafrost_master = []
...
################# loop through in-situ files ###############
    				...
    				for path in pathlist_sorted:
    					...
    					if ((lon_cen == -180.0) or (lon_cen == 180)): #longitude values range between -179.5 and 179.5, so we are converting -180/180 to 0 here
    						lon_cen = 0
    					...
    					print("Lat Cen:",lat_cen)
    					print("Lon Cen:", lon_cen)    
    					...
    					ERA5_MAAT = ERA5_MAAT_fil[ERA5_air] - 273.15
...
###### warm Season (Soil Temp <= -2) #####
    					...
    					#print(dframe_stn2)
    					dframe_stn3 = dframe_stn2.reindex(dt_rng,fill_value=np.nan)
    					#print(dframe_stn3)

    					dframe_stn3.insert(0,'Grid Cell', gcell)
    					dframe_stn3.insert(1, 'Central Lat', lat_cen)
    					dframe_stn3.insert(2, 'Central Lon', lon_cen)
    					...
    					dframe_stn3['Continent'] = continent
    					dframe_stn3['RS 2002 Permafrost'] = RS_2002_permafrost
    					dframe_stn3['Brown 1970 Permafrost'] = Brown_1970_permafrost
    					...
    					continent_master.append(dframe_stn3['Continent'].values.tolist())
    					RS_2002_permafrost_master.append(dframe_stn3['RS 2002 Permafrost'].values.tolist())
    					Brown_1970_permafrost_master.append(dframe_stn3['Brown 1970 Permafrost'].values.tolist())
    				...
    				continent_master = [i for sub in continent_master for i in sub]
    				RS_2002_permafrost_master = [i for sub in RS_2002_permafrost_master for i in sub]
    				Brown_1970_permafrost_master = [i for sub in Brown_1970_permafrost_master for i in sub]
    				...
    				dframe_master['Continent'] = continent_master
    				dframe_master['RS 2002 Permafrost'] = RS_2002_permafrost_master
    				dframe_master['Brown 1970 Permafrost'] = Brown_1970_permafrost_master
    				#dframe_master.dropna(inPlace=True)
    				print(dframe_master)

    				timeseries_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_timeseries/new_data/CLSM_res/subset/'+str(remap_type)+"_"+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_'+str(thr_m)+'_data_CMOS_CLSM_subset_permafrost.csv'])
