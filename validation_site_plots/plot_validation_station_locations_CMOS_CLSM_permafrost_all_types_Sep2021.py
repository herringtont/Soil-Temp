...
dframe_con_top = pd.DataFrame(data=gcell_master_top_30_con,columns=['Grid Cell'])
dframe_con_top['Lat'] = lat_master_top30_con
dframe_con_top['Lon'] = lon_master_top30_con

dframe_dis_top = pd.DataFrame(data=gcell_master_top_30_dis,columns=['Grid Cell'])
dframe_dis_top['Lat'] = lat_master_top30_dis
dframe_dis_top['Lon'] = lon_master_top30_dis

dframe_none_top = pd.DataFrame(data=gcell_master_top_30,columns=['Grid Cell'])
...
plt.text(
plt.tight_layout()
plt_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/plots/validation_sites/new_data/CLSM_res/validation_sites_remapcon_zscore_all_layers_thr100_CMOS_CLSM_RS_2002_all_permafrost_types_top30_BEST_Sep2021.png'])
fig.savefig(plt_fil)
plt.close()
