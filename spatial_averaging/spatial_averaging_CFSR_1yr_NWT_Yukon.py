...
geom_dir = "/mnt/data/users/herringtont/soil_temp/In-Situ/spatial_join/CFSR/"

olr = ['outliers','zscore','IQR']
lyr = ['top_30cm','30_299.9']
thr = ['0','25','50','75','100']
rmp_type = ['con']
...
############ loop through soil layer ##############    
    	for j in lyr:
    		...
    		if (j == 'top_30cm'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_top30_nn_CFSR.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_top30_bil_CFSR.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_top30_con_CFSR.csv"])
    		if (j == '30_299.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L7_nn_CFSR.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L7_bil_CFSR.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L7_con_CFSR.csv"])
...
############# loop through missing threshold ##########    	
    		for k in thr:
    			thrk = k
    			two_year_fil = "".join(["/mnt/data/users/herringtont/soil_temp/sites_1yr/CFSR/sites_1yr_",olri,"_",lyrj,"_",thrk,".csv"])
    			#print(two_year_fil)
    	...
    	for k in lyr:
    		...
    		for l in thr:
    			...
    			for m in grid_new_uq:
    				...
    				lon_g = lon_g.iloc[0]
    				if (-109 >= lon_g >= -136): #skip all grid cells in the NWT
    					continue  				
################# get unique dates to reindex soil temperature against ##########################
    				for n in sites:
    					#print("the threshold is:",thrl)
    					sitid = n
    					stemp_fil = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/",olrj,"/",lyrk,"/monthly_average/thr_",thrl,"/CFSR/site_",str(sitid),".csv"])
    					#print(stemp_fil)
...
################# reindex all files within a grid cell onto a common time axis ######################
    				...
    				for o in sites:
    					...
    					if (sitid2 == 6):
    						continue
    					stemp_fil2 = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/",olrj,"/",lyrk,"/monthly_average/thr_",thrl,"/CFSR/site_",str(sitid2),".csv"])
    					#print(stemp_fil2)    					
...
################## do the spatial averaging ###################
    				if(len(dframe_gcell) > 4):
    					...
    					ofil = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_1yr/remap",rmpi,"/no_outliers/",olrj,"/",lyrk,"/thr_",thrl,"/CFSR/Sep2021_NWT_Yukon/grid_",str(gcell),"_Sep2021_NWT_Yukon.csv"])
    				...
    				if(len(dframe_gcella) > 4):
    					...
    					ofila = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_anom_1yr/remap",rmpi,"/no_outliers/",olrj,"/",lyrk,"/thr_",thrl,"/CFSR/Sep2021_NWT_Yukon/grid_",str(gcell),"_anom_Sep2021_NWT_Yukon.csv"])
