...
lyr = ['top_30cm','30_299.9']#['0_9.9','10_29.9','30_99.9','100_299.9','300_deeper','top_30cm']
thr = ['0','25','50','75','100']
rmp_type = ['nn','bil','con']
...
############ loop through soil layer ##############    
    	for j in lyr:
    		...
    		if (j == '0_9.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L1_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L1_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L1_con.csv"])
    		if (j == '10_29.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L2_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L2_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L2_con.csv"])
    		if (j == '30_99.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L3_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L3_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L3_con.csv"])
    		if (j == '100_299.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L4_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L4_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L4_con.csv"])
    		if (j == '300_deeper'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L5_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L5_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L5_con.csv"])
    		if (j == 'top_30cm'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_top30_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_top30_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_top30_con.csv"])
    		if (j == '30_299.9'):
    			if(h == "nn"):
    				geom_fil = "".join([geom_dir,"geometry_L7_nn.csv"])
    			if(h == "bil"):
    				geom_fil = "".join([geom_dir,"geometry_L7_bil.csv"])
    			if(h == "con"):
    				geom_fil = "".join([geom_dir,"geometry_L7_con.csv"])
...
############# loop through missing threshold ##########    	
    		for k in thr:
    			thrk = k
    			two_year_fil = "".join(["/mnt/data/users/herringtont/soil_temp/sites_2yr/sites_2yr_",olri,"_",lyrj,"_",thrk,".csv"])
    			print(two_year_fil)
    			dframe_2yr = pd.read_csv(two_year_fil)
    			sites_2yr = dframe_2yr['Sites'].values.tolist()		

    			for l in range(0,len(sites_2yr)):
    				sitid = sites_2yr[l]
    				print(sitid)
    				site_grid = dframe_geom[dframe_geom['site'] == int(sitid)]
    				grid_cell = site_grid['Grid Cell'].values.tolist()
    				print(grid_cell)
    				lat = site_grid['Lat Cen'].values.tolist()
    				print(lat)
    				lon = site_grid['Lon Cen'].values.tolist()
    				print(lon)
...
################# get unique dates to reindex soil temperature against ##########################
    				for n in sites:
    					...
    					sitid = n
    					stemp_fil = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/",olrj,"/",lyrk,"/monthly_average/thr_",thrl,"/site_",str(sitid),".csv"])
    					#print(stemp_fil)
...
################# reindex all files within a grid cell onto a common time axis ######################
    				...
    				for o in sites:
    					...
    					if (sitid2 == 6):
    						continue
    					stemp_fil2 = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/",olrj,"/",lyrk,"/monthly_average/thr_",thrl,"/site_",str(sitid2),".csv"])
    					#print(stemp_fil2)    					
...
################## do the spatial averaging ###################
    				if(len(dframe_gcell) > 4):
    					...
    					ofil = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average/remap",rmpi,"/no_outliers/",olrj,"/",lyrk,"/thr_",thrl,"/grid_",str(gcell),".csv"])
    				...
    				if(len(dframe_gcella) > 4):
    					...
    					ofila = "".join(["/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_anom/remap",rmpi,"/no_outliers/",olrj,"/",lyrk,"/thr_",thrl,"/grid_",str(gcell),"_anom.csv"])
