    ...
    for j in naive_type:
    	...
    	for k in olr:
    		...
    		for l in lyr:
    			...
    			for m in thr:
    				thr_m = m
    				insitu_dir =  ''.join(['/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_anom_1yr/'+str(remap_type)+'/no_outliers/'+str(olr_k)+'/'+str(in_situ_layer)+'/thr_'+str(thr_m)+'/CLSM/Sep2021_NWT/'])
...
################# loop through in-situ files ###############
    					...
    					for path in pathlist_sorted:
    						...
    						#if (-109 >= lon_cen >= -136): #skip grid cells based in NWT
    							#continue
...
########################### Create Scatterplot Dataframe ############################
    					...
    					cold_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/sites_1yr/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_cold_season_temp_master_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021_NWT.csv'])
    					path_cold = pathlib.Path(cold_fil)
    					path_cold.parent.mkdir(parents=True,exist_ok=True)
    					dframe_cold_season_temp_master.to_csv(cold_fil,index=False)


    					warm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/sites_1yr/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_warm_season_temp_master_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021_NWT.csv'])
    					path_warm = pathlib.Path(warm_fil)
    					path_warm.parent.mkdir(parents=True,exist_ok=True)
    					...
    					scatter_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/sites_1yr/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_scatterplot_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021_NWT.csv'])
    					path_scatter = pathlib.Path(warm_fil)
    					path_scatter.parent.mkdir(parents=True,exist_ok=True)
