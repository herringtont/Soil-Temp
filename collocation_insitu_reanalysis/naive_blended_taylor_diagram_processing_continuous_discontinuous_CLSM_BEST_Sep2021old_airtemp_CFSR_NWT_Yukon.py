...
for i in rmp_type:
    ...
    BEST_fi_air = "".join(['/mnt/data/users/herringtont/soil_temp/reanalysis/2m_AirTemp/rename/land_only/common_grid_CFSR/remapcon/masked_files/degC/GHCN_2m_air.nc'])
    ...
    ERA5_MAAT_fi = ''.join(['/mnt/data/users/herringtont/soil_temp/reanalysis/2m_AirTemp/rename/land_only/common_grid_CFSR/'+str(remap_type)+'/masked_files/MAAT/GHCN_2m_air_MAAT_1981_2010_clim.nc'])
    ...
    for j in naive_type:
    	...
    	for k in olr:
    		...
    		for l in lyr:
    			...
    			for m in thr:
    				thr_m = m
    				insitu_dir =  ''.join(['/mnt/data/users/herringtont/soil_temp/In-Situ/All/spatial_average_anom_1yr/'+str(remap_type)+'/no_outliers/'+str(olr_k)+'/'+str(in_situ_layer)+'/thr_'+str(thr_m)+'/CFSR/Sep2021_NWT_Yukon/'])
...
########################### Create Scatterplot Dataframe ############################
    					...
    					cold_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CFSR_res/'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_cold_season_temp_master_GHCN_subset_permafrost_Sep2021_NWT_Yukon_airtemp_CFSR_NWT_Yukon.csv'])
    					dframe_cold_season_temp_master.to_csv(cold_fil,index=False)


    					warm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CFSR_res/'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_warm_season_temp_master_GHCN_subset_permafrost_Sep2021_NWT_Yukon_airtemp_CFSR_NWT_Yukon.csv'])
    					...
    					scatter_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_scatterplot_GHCN_subset_permafrost_Sep2021_NWT_Yukon_air_CFSR.csv'])
