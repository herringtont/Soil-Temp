...
for i in rmp_type:
    ...
    air_dir = ''.join(['/mnt/data/users/herringtont/soil_temp/reanalysis/2m_AirTemp/rename/land_only/common_grid_CLSM/'+str(remap_type)+'/masked_files/'])
    ...
    ERA5_air_fi = "".join([air_dir,"BEST_2m_air.nc"])
    ERA5_MAAT_fi = ''.join([air_dir,'MAAT/BEST_2m_air_MAAT_1981_2010_clim.nc'])
...
################# loop through in-situ files ###############
    					...
    					for path in pathlist_sorted:
    						...
    						ERA5_tair = ERA5_air_fil[ERA5_air]
    						ERA5_MAAT = ERA5_MAAT_fil[ERA5_air]
...
###### Cold Season (Soil Temp <= -2) #####
    						...
    						for x in cold_idx:
    							...
    							date_temp_cold_season.append(date_x)
    						...
    						print(date_temp_cold_season)
    						dframe_cold_season_temp = pd.DataFrame(data = station_temp_cold_season, columns=['Station'])
    						dframe_cold_season_temp.insert(0,value=date_temp_cold_season,column='Date')
...
###### warm Season (Soil Temp <= -2) #####
    						...
    						for x in warm_idx:
    							date_x = date_temp_master[x].tolist()
    							date_temp_warm_season.append(date_x)
    							station_x = station_temp_master[x].tolist()
    						...
    						dframe_warm_season_temp.insert(0,value=date_temp_warm_season,column='Date')
...
########################### Create Scatterplot Dataframe ############################
    						...
    						date_data_master_cold_temp.append(dframe_cold_season_temp['Date'].values.tolist())
    						date_data_master_warm_temp.append(dframe_warm_season_temp['Date'].values.tolist())
    					...
    					date_data_master_cold_temp = [i for sub in date_data_master_cold_temp for i in sub]
    					date_data_master_warm_temp = [i for sub in date_data_master_warm_temp for i in sub]
    					...
    					dframe_cold_season_temp_master['Date'] = date_data_master_cold_temp
    					...
    					dframe_warm_season_temp_master['Date'] = date_data_master_warm_temp
    					...
    					cold_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_cold_season_temp_master_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021.csv'])
    					dframe_cold_season_temp_master.to_csv(cold_fil,index=False)


    					warm_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_warm_season_temp_master_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021.csv'])
    					...
    					scatter_fil = ''.join(['/mnt/data/users/herringtont/soil_temp/naive_blend_taylor_metrics/new_data/CLSM_res/subset/'+str(remap_type)+'_'+str(naive_type_j)+'_'+str(olr_k)+'_'+str(lyr_l)+'_thr_'+str(thr_m)+'_dframe_scatterplot_BEST_'+str(temp_thr_o)+'_CMOS_CLSM_subset_permafrost_Sep2021.csv'])
