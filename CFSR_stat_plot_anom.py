...
##################### set file locations #################
CFSR_fi = "/mnt/data/users/herringtont/soil_temp/reanalysis/monthly/remap/rename/CFSR_stats/anom/CFSR_deseasonalized.nc"
CFSR2_fi = "/mnt/data/users/herringtont/soil_temp/reanalysis/monthly/remap/rename/CFSR_stats/anom/CFSR2_deseasonalized.nc"
...
################### select nearest grid cell from each netCDF file #####################
    CFSR_stemploc_L1 = CFSR_stemp_L1.sel(lon=lonf,lat=latf,method='nearest')
    CFSR2_stemploc_L1 = CFSR2_stemp_L1.sel(lon=lonf,lat=latf,method='nearest')
    
    CFSR_stemploc_L2 = CFSR_stemp_L2.sel(lon=lonf,lat=latf,method='nearest')
    CFSR2_stemploc_L2 = CFSR2_stemp_L2.sel(lon=lonf,lat=latf,method='nearest')    
    
    CFSR_stemploc_L3 = CFSR_stemp_L3.sel(lon=lonf,lat=latf,method='nearest')
    CFSR2_stemploc_L3 = CFSR2_stemp_L3.sel(lon=lonf,lat=latf,method='nearest')
    
    CFSR_stemploc_L4 = CFSR_stemp_L4.sel(lon=lonf,lat=latf,method='nearest')
    CFSR2_stemploc_L4 = CFSR2_stemp_L4.sel(lon=lonf,lat=latf,method='nearest')
...
#    	mxrg = 17 
	...
	#ax.set_xlabel('Date/Time')
    	...
    	ax.legend()
    plt.tight_layout()   
    L1fil = "".join(["/mnt/data/users/herringtont/soil_temp/plots/CFSR_anom/site"+str(min_site)+"_site"+str(max_site),"_L4.png"])
    print(L1fil)
    plt.savefig(L1fil)
    plt.close()
#
