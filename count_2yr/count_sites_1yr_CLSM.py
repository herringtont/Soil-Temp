...
def load_pandas(file_name):
    ...
    if (diff2 >= 12 and len(jan) >=1 and len(feb) >=1 and len(mar) >=1 and len(apr) >=1 and len(may) >=1 and len(jun) >=1 and len(jul) >=1 and len(aug) >=1 and len(sep) >=1 and len(obr) >=1 and len(nov) >=1 and len(dec) >=1):
...
for i in olr:
    ...
    for j in lyr:
    	...
    	for k in thr:
    		...
    		if (len(df_yr_new) > 0):
    			print("Outlier:",i,", Layer:",j,", Threshold:",k)
    			print("Sites with at least 1 years:",len(df_yr_new))
    			yr_ofil = "".join(["/mnt/data/users/herringtont/soil_temp/sites_1yr/CLSM/sites_1yr_",str(olri),"_",str(lyrj),"_",str(thrk),".csv"])
