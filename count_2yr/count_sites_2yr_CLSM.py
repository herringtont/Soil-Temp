...
def load_pandas(file_name):
    ...
    #print(layer)
    thresh = file_name.split("/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/"+outlr+"/"+layer+"/monthly_average/thr_")[1].split("/CLSM/")[0]
    #print(thresh)
...
#set files and directories
    ...
    for i in olr:
    	...
    	for j in dep:
    		for k in thr:
    			pthl2 = [directory,str(i),"/",str(j),"/","monthly_average","/","thr_",str(k),"/CLSM/"]	
    			pthl3 = "".join(pthl2)
    			#print(pthl3)
    ...
    for j in lyr:
    	...
    	for k in thr:
    		...
    		if (len(df_CFSR_new) > 0):
    			...
    			CFSR_ofil = "".join(["/mnt/data/users/herringtont/soil_temp/CFSR_overlap/CLSM/CFSR_overlap_sites_",str(olri),"_",str(lyrj),"_",str(thrk),".csv"])
    			df_CFSR_new.to_csv(CFSR_ofil,index=False)
    		if (len(df_yr_new) > 0):
    			print("Outlier:",i,", Layer:",j,", Threshold:",k)
    			print("Sites with at least 2 years:",len(df_yr_new))
    			yr_ofil = "".join(["/mnt/data/users/herringtont/soil_temp/sites_2yr/CLSM/sites_2yr_",str(olri),"_",str(lyrj),"_",str(thrk),".csv"])
