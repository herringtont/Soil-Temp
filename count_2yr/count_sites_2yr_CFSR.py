...
def load_pandas(file_name):
    ...
    #print(layer)
    thresh = file_name.split("/mnt/data/users/herringtont/soil_temp/In-Situ/All/layer_average/no_outliers/"+outlr+"/"+layer+"/monthly_average/thr_")[1].split("/CFSR/")[0]
    #print(thresh)
    ...
    #print("Number of valid months:"+str(diff2))
    #print("Jan data:"+str(jan_l)+" months")
    #print("Feb data:"+str(feb_l)+" months")
    #print("Mar data:"+str(mar_l)+" months")
    #print("Apr data:"+str(apr_l)+" months")
    #print("May data:"+str(may_l)+" months")
    #print("Jun data:"+str(jun_l)+" months")
    #print("Jul data:"+str(jul_l)+" months")
    #print("Aug data:"+str(aug_l)+" months")
    #print("Sep data:"+str(sep_l)+" months")
    #print("Oct data:"+str(obr_l)+" months")
    #print("Nov data:"+str(nov_l)+" months")
    #print("Dec data:"+str(dec_l)+" months")
    ...
    if (diff2 >= 25 and len(jan) >= 2 and len(feb) >=2 and len(mar) >=2 and len(apr) >= 2 and len(may) >= 2 and len(jun) >= 2 and len(jul) >= 2 and len(aug) >= 2 and len(sep) >= 2 and len(obr) >= 2 and len(nov) >=2 and len(dec) >= 2):
...
#set files and directories
    ...
    for i in olr:
    	...
    	for j in dep:
    		for k in thr:
    			pthl2 = [directory,str(i),"/",str(j),"/","monthly_average","/","thr_",str(k),"/CFSR/"]	
    			pthl3 = "".join(pthl2)
    			#print(pthl3)
    ...
    for j in lyr:
    	...
    	for k in thr:
    		...
    		if (len(df_CFSR_new) > 0):
    			...
    			CFSR_ofil = "".join(["/mnt/data/users/herringtont/soil_temp/CFSR_overlap/CFSR/CFSR_overlap_sites_",str(olri),"_",str(lyrj),"_",str(thrk),".csv"])
    			df_CFSR_new.to_csv(CFSR_ofil,index=False)
    		if (len(df_yr_new) > 0):
    			print("Outlier:",i,", Layer:",j,", Threshold:",k)
    			print("Sites with at least 2 years:",len(df_yr_new))
    			yr_ofil = "".join(["/mnt/data/users/herringtont/soil_temp/sites_2yr/CFSR/sites_2yr_",str(olri),"_",str(lyrj),"_",str(thrk),".csv"])
